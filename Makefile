# This Makefile is for the Dao::Map::Helper extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/Dao/Map/Helper.pm]
#     AUTHOR => q[Arjun Surendra <arjun.surendra04@gmail.com>]
#     BUILD_REQUIRES => {  }
#     EXE_FILES => [q[bin/dao-map-helper]]
#     LICENSE => q[perl]
#     NAME => q[Dao::Map::Helper]
#     PL_FILES => {  }
#     PREREQ_PM => { Test::More=>q[0] }
#     VERSION_FROM => q[lib/Dao/Map/Helper.pm]
#     clean => { FILES=>q[Dao-Map-Helper-*] }
...
INST_LIBDIR      = $(INST_LIB)\Dao\Map
INST_ARCHLIBDIR  = $(INST_ARCHLIB)\Dao\Map
...
TO_INST_PM = lib/Dao/Map/Helper.pm

PM_TO_BLIB = lib/Dao/Map/Helper.pm \
    blib\lib\Dao\Map\Helper.pm
...
DISTNAME = Dao-Map-Helper
DISTVNAME = Dao-Map-Helper-0.01
...
Helper.def: Makefile.PL
    $(PERLRUN) -MExtUtils::Mksymlists \
     -e "Mksymlists('NAME'=>\"Dao::Map::Helper\", 'DLBASE' => '$(BASEEXT)', 'DL_FUNCS' => {  }, 'FUNCLIST' => [], 'IMPORTS' => {  }, 'DL_VARS' => []);"
...
pure_all :: $(INST_SCRIPT)\dao-map-helper
    $(NOECHO) $(NOOP)

realclean ::
    $(RM_F) \
      $(INST_SCRIPT)\dao-map-helper

$(INST_SCRIPT)\dao-map-helper : bin/dao-map-helper $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
    $(NOECHO) $(RM_F) $(INST_SCRIPT)\dao-map-helper
    $(CP) bin/dao-map-helper $(INST_SCRIPT)\dao-map-helper
    $(FIXIN) $(INST_SCRIPT)\dao-map-helper
    -$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)\dao-map-helper
...
metafile : create_distdir
    ...
    $(NOECHO) $(ECHO) "name:               Dao-Map-Helper" >> META_new.yml
    $(NOECHO) $(ECHO) "version:            0.01" >> META_new.yml
    $(NOECHO) $(ECHO) "abstract:           Simplify the creation of DAO (Data Access Objects). Kind of a low level ORM, where you can still use SQL and then map the result set to the class objects." >> META_new.yml
    $(NOECHO) $(ECHO) "author:" >> META_new.yml
    $(NOECHO) $(ECHO) "    - Arjun Surendra <arjun.surendra04@gmail.com>" >> META_new.yml
...
ppd :
    ...
    $(NOECHO) $(ECHO) "    <ABSTRACT>Simplify the creation of DAO (Data Access Objects). Kind of a low level ORM, where you can still use SQL and then map the result set to the class objects.</ABSTRACT>" >> $(DISTNAME).ppd
    $(NOECHO) $(ECHO) "    <AUTHOR>Arjun Surendra &lt;arjun.surendra04@gmail.com&gt;</AUTHOR>" >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
    $(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e "pm_to_blib({{@ARGV}, '$(INST_LIB)\auto', q[$(PM_FILTER)], '$(PERM_DIR)')" -- \
      lib/Dao/Map/Helper.pm blib\lib\Dao\Map\Helper.pm
